---
source: https://helpdesk.dooray.com/v2/wapi/shared-tree/9wWo-xwiR66BO5LGshgVTg/pages/2939987647631384419
pageId: "2939987647631384419"
subject: "서비스 API"
...
* 공공 업무망 클라우드
    ...
    * [https://api.gov-dooray.co.kr](https://api.gov-dooray.co.kr)
* 금융 클라우드
...
| 헤더 이름 | 설명 |
| ----- | --- |
| X-RateLimit-Remaining | 현재 남아있는 토큰 수 |
| X-RateLimit-Requested-Tokens | 이번 요청에서 사용된 토큰 수 |
| X-RateLimit-Burst-Capacity | 최대 토큰 용량 (버스트 허용량) |
| X-RateLimit-Replenish-Rate | 1초당 보충되는 토큰 수 |
...
| # | name | type | available value | default | desc |
| --- | ---- | ---- | --------------- | ------- | ---- |
| 1 | calendar-id | path | number | - | 캘린더 id |
...
* 캘린더 마스터 멤버는 수정할 수 없습니다.
* 마스터 외 캘린더에 속한 멤버를 설정합니다. (override 방식으로 동작)
...
* delegatee - 마스터 대신 일정 관리
* all - 캘린더 및 일정 관리
* read\_write - 일정 관리
* view - 일정 조회 (상세 정보 확인)
* opaque\_view - 일정 있음/없음만 조회 (상세 정보 숨김)

organizationMemberId / emailAddress

* 두레이에 존재하지 않는 organizationMemberId 혹은 distributionList.emailAddress(그룹메일 이메일 주소) 라면, 무시 됩니다.
...
* 위키 페이지의 순서를 변경하거나, 다른 위키 또는 다른 부모 페이지의 하위로 이동합니다.
* 하위 페이지까지 함께 이동할지 여부를 선택할 수 있습니다.

#### Request

* Path

    * {wiki-id} - 이동시킬 페이지가 속한 위키 id
    * {page-id} - 이동시킬 위키 페이지 id
* Body

```javascript
{
    "targetWikiId": "",       /* 이동 대상 위키 id (옵션)*/
    "targetParentPageId": "", /* 이동 대상 부모 페이지 id (필수) */
    "withChildren": true,                        /* 하위 페이지까지 함께 이동 여부 (default: true) */
    "beforePageId": ""        /* 이 페이지 바로 뒤에 위치시킴 (옵션) */
}
```

필드 설명

* beforePageId - 정렬 기준 이전 페이지 아이디. null이면 정렬 변경 없음. 0이면 맨 앞으로 이동
...
### DELETE /wiki/v1/wikis/{wiki-id}/pages/{page-id}
...
* Path
    ...
    * {wiki-id} - 삭제할 페이지가 속한 위키 id
    * {page-id} - 삭제할 위키 페이지 id
...
`{pageId}` 에 해당하는 위키 페이지에 댓글을 추가합니다.
content 형식은 `text/x-markdown`(마크다운) 형식만 제공합니다.
    ...
    "result": [
        {
            ...
            "includeDescendants": true // true - 하위 페이지 포함 공유, false - 본 페이지만 공유
        }
    ],
    "totalCount": 1
}
...
```
    type=general            /* form-data 내에 포함되어야 함. general | inline_image */
    file                    /* file content */
```
...
* `type` 종류
    ...
    * `general` - 일반 첨부 파일
    * `inline_image` - 위키 페이지 본문에 삽입할 인라인 이미지

        * 업로드 후 응답의 `$.result.name` 과 `$.result.attachFileId` 를 사용하여 위키 페이지 본문 내에 다음과 같이 이미지를 삽입할 수 있습니다.

        ```
        ![{$.result.name}](/wikis/{wiki-id}/files/{$.result.attachFileId})
        
        e.g.) ![image.png](/share/tree/9wWo-xwiR66BO5LGshgVTg/pages/2939987647631384419/attach-files/4332191948198981118)
        ```
...
### POST /messenger/v1/channels/{channel-id}/logs/{log-id}/reply

* 대화방의 특정 메시지에 답장(reply)을 보냅니다.
...
### POST /messenger/v1/channels/{channel-id}/logs/{log-id}/threads/create-and-send

* 특정 메시지(log-id)를 기반으로 글타래(thread)를 생성하고 메시지를 전송합니다.
* `{log-id}`는 글타래를 생성할 대상 메시지의 log-id입니다.
...
```javascript
{
    ...
    "result": {
        "id": "3986497071236383013",              // 쓰레드 메시지를 전송할  메시지의 log-id
        "channelId": "3986497069711082184"
    }
}
```
